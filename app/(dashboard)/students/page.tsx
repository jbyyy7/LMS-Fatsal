...
import { Users, BookOpen, Mail, GraduationCap, Search } from 'lucide-react';
    ...
    <div className="p-6">
      <div className="mb-6">
        ...
        <p className="text-gray-600 mt-1">
          {profile?.role === 'Teacher' ? 'Siswa yang terdaftar di kursus Anda' : 'Daftar semua siswa'}
        </p>
      </div>
        ...
        <div className="bg-white p-4 rounded-lg shadow">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-2xl font-bold">
                {students.length > 0 
                  ? Math.round(students.reduce((acc, s) => acc + (s._enrollments || 0), 0) / students.length * 10) / 10
                  : 0
                }
              </p>
            </div>
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Kursus Diikuti</th>
            ...
            {filteredStudents.map((student) => (
              <tr key={student.id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 text-sm text-gray-600">
                  {student.class?.name || <span className="text-gray-400">Belum ada kelas</span>}
                </td>
                <td className="px-6 py-4">
                  <div className="flex items-center gap-2">
                    <BookOpen className="w-4 h-4 text-gray-400" />
                    <span className="text-sm font-semibold text-gray-900">
                      {student._enrollments || 0} kursus
                    </span>
                  </div>
                </td>
                <td className="px-6 py-4">
        ...
        {filteredStudents.length === 0 && (
          <div className="text-center py-12">
            ...
            <p className="text-gray-600">
              {search ? 'Coba kata kunci lain' : 'Siswa akan muncul di sini setelah terdaftar'}
            </p>
          </div>
        )}
      </div>
